...
server.on("message", async (msg, rinfo) => {
  ...
  let domainParts;
  let askedRecord;
  [recordsResult, qt, domainParts, askedRecord] = await getRecords(
    msg.slice(12),
  );

  let askedRecords = recordsResult[qt].filter(
    (ele) => ele.name === askedRecord,
  );
  ...
  server.send(
    [TID, FLAGS, QDCOUNT, ANCOUNT, NSCOUNT, ARCOUNT, domainQuestion, dnsBody],
    rinfo.port,
  );
});
